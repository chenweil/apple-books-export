...
当前 AppKit 版本: `v0.1.5` (`build 6`, unsigned)。应用重新激活时会重新读取 Apple Books 数据，也可以在“设置…”中配置低频自动刷新，变更记录见
...
产物位于仓库根目录的 `dist/Books-Exporter-0.1.5-unsigned.dmg`。也可以覆盖版本号：

```bash
APP_VERSION=0.1.6 BUILD_VERSION=7 ./Scripts/package-dmg.sh
```
